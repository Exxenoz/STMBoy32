...
// Lock button for lockTime in ms (multiply lockTime by timer frequency / 1000 to get ms).
void Input_Lock(Input_Button_ID_t id, time_t lockTime)
{
    Input_Locks[id].IsLocked    = true;
    Input_Locks[id].LockedFor   = ((INPUT_LOCK_TIM_FREQ / (INPUT_LOCK_TIM_PRESCALER + 1)) / 1000) * lockTime;
